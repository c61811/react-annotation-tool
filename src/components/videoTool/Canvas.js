		...
		objects.slice().reverse().forEach( obj => {
			...
			for( let i = 0; i < trajectories.length; i++){
				...
				if(played >= trajectories[i].time){
					...
					layerItems.push(<Group x={x} y={y} key={obj.name} name={obj.name} draggable={true} onDragMove={this.handle} onMouseDown={this.handleGroupMouseDown} onDragEnd={this.handleGroupDragEnd} onDragStart={this.handleGroupDragStart} onDragMove={this.handleGroupDragMove}>{name}{rect}{dots}</Group>)
					break;
				}
			}
		});
